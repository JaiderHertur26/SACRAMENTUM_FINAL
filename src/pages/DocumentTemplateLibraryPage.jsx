...
import { Copy, FileText, History, Printer, Search, ShieldCheck, Sparkles, PencilLine, Save } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-[360px_1fr] gap-7">
          <aside className="bg-white border rounded-3xl overflow-hidden h-fit lg:sticky lg:top-24">
            ...
            <div className="max-h-[65vh] overflow-y-auto divide-y">
              {loading ? <div className="p-8 text-center text-slate-400">Cargando...</div> : filtered.length === 0 ? <div className="p-8 text-center text-slate-400">No hay plantillas activas.</div> : filtered.map((t) => <button key={t.id} type="button" onClick={() => setSelectedId(t.id)} className={`w-full text-left p-4 transition ${selectedId === t.id ? 'bg-blue-50 border-l-4 border-blue-600' : 'hover:bg-slate-50'}`}><div className="flex items-center justify-between gap-2"><div className="text-[9px] font-black tracking-widest uppercase text-blue-500">{t.category || 'Documento'} · {t.code || 'S/C'}</div>{t.is_legacy&&<span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[8px] font-black uppercase tracking-wider text-amber-700">Legacy original</span>}</div><div className="font-black text-sm text-slate-900 mt-1">{t.name}</div></button>)}
            </div>
          </aside>
          ...
          <section className="space-y-6">
            {!selected ? <div className="bg-white border border-dashed rounded-3xl p-16 text-center text-slate-400"><FileText className="w-12 h-12 mx-auto mb-4" />Selecciona una plantilla.</div> : <>
              <div className="bg-white border rounded-3xl p-6">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6"><div><div className="flex flex-wrap items-center gap-2"><div className="text-[10px] font-black uppercase tracking-widest text-blue-500">{selected.category || 'Documento'} · {selected.code}</div>{selected.is_legacy&&<span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[8px] font-black uppercase tracking-wider text-amber-700"><History className="h-3 w-3"/>Texto histórico preservado</span>}</div><h2 className="text-2xl font-black text-slate-900 mt-1">{selected.name}</h2>{selected.is_legacy&&<p className="mt-2 max-w-2xl text-xs leading-relaxed text-slate-500">Esta versión conserva literalmente la redacción de SACRAMENTA PLUS. Puede crear una versión moderna sin borrar ni alterar el original histórico.</p>}</div><div className="flex flex-wrap gap-2">{canManage && <Button variant="outline" onClick={beginEdit}><PencilLine className="w-4 h-4 mr-2" />Nueva versión</Button>}<Button variant="outline" onClick={copy}><Copy className="w-4 h-4 mr-2" />Copiar</Button><Button onClick={print} className="bg-blue-700 text-white"><Printer className="w-4 h-4 mr-2" />Imprimir</Button></div></div>
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 px-5 py-4 text-xs text-slate-600"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700"/><div><b>Gobierno documental:</b> los marcadores que todavía aparezcan como <code>&lt;Campo&gt;</code> indican datos no completados. Una plantilla legacy nunca se sobrescribe: cualquier modernización crea una versión nueva y mantiene la procedencia, código histórico y texto original.</div></div></div>
            </>}
          </section>
        </div>
      </div>
