...
import { Search, Eye, FileText, ShieldAlert, BookOpen, ArrowRight, Loader2, Cloud, LockKeyhole } from 'lucide-react';
        ...
        { header: 'Fecha', render: (row) => <span className="text-xs font-medium text-slate-500">{row.decreeDate}</span> },
        {
            ...
            render: (row) => (
                <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-9 w-9 text-blue-600 hover:bg-blue-50 rounded-xl" onClick={() => { setSelectedDecree(row); setViewModalOpen(true); }} title="Consultar decreto emitido por Cancillería"><Eye className="w-4 h-4" /></Button>
                </div>
            )
        }
    ];
        ...
        <DashboardLayout entityName={user?.parishName || "Parroquia"}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div><h1 className="text-3xl font-black text-slate-900 font-serif">Archivo de Decretos</h1><p className="text-slate-500 text-sm font-medium uppercase text-[10px] tracking-widest">Correcciones de Bautismo</p></div>
                </div>
            ...
            >
                <TabsList className="grid w-full grid-cols-3 bg-slate-100 p-1 rounded-2xl h-14">
                    ...
                    <TabsTrigger value="confirmaciones" className="rounded-xl font-bold uppercase text-[10px] tracking-widest cursor-pointer text-slate-500 hover:bg-slate-200/50 transition-all">Confirmaciones</TabsTrigger>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-8 bg-slate-50/50 border-b border-slate-100">
                    <div className="relative max-w-md group"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 group-focus-within:text-[#4B7BA7] transition-colors" /><Input placeholder="Buscar por acta, decreto o nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-12 py-7 text-sm rounded-2xl border-slate-200 bg-white shadow-sm focus:ring-4 focus:ring-blue-500/5 transition-all" /></div>
                </div>

                {loading ? (
                    <div className="py-24 text-center"><Loader2 className="w-10 h-10 animate-spin text-[#4B7BA7] mx-auto mb-4" /><p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Descargando...</p></div>
