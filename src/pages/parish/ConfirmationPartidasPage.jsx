...
const InfoBox = ({ data, marginalNotes = [] }) => {
    ...
    const baptismArchiveReference = [
        ...
        data.numeroBautismo ? `N:${String(data.numeroBautismo).padStart(4,'0')}` : ''
    ].filter(Boolean).join(' · ') || '---';

    const explicitDaFe = String(data.daFe || '').trim();
    const resolvedDaFe = explicitDaFe
        ? explicitDaFe
        : (data.legacyDaFeCode ? `CÓDIGO LEGADO: ${data.legacyDaFeCode}` : '---');
    const resolvedMinister = String(data.ministro || '').trim() || '---';
    const activeNotesText = marginalNotes.length
        ? marginalNotes.map(n => n.content).filter(Boolean).join('\n\n')
    ...
    return (
        <div className="mt-8 border border-slate-200/80 rounded-[2.5rem] overflow-hidden shadow-2xl bg-white animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="bg-[#315878] px-8 py-5 flex justify-between items-center">
            ...
            <div className="p-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
...
const ConfirmationPartidasPage = () => {
    ...
    useEffect(() => {
        ...
        const loadPrintProfile = async () => {
            ...
            try {
                const cloudProfile = await getParishPrintProfile(parishId);
                if (mounted) setParishPrintData({ ...legacyProfile, ...(cloudProfile || {}) });
            } catch (error) {
                console.warn('No fue posible cargar el perfil institucional de Confirmación:', error);
                if (mounted) setParishPrintData(legacyProfile);
            }
        };
    ...
    const fetchRecords = async () => {
        ...
        try {
            const processedData = await fetchConfirmationsFromSource(parishId);
    ...
    useEffect(() => {
        ...
        listMarginalNotesForRecord({
            ...
            legacyInlineNote: selectedPartida.notaMarginal || ''
        }).then(notes => {
            if (mounted) setSelectedMarginalNotes(notes);
        }).catch(() => {
            if (mounted) setSelectedMarginalNotes([]);
        });
        return () => { mounted = false; };
            ...
            <div className="max-w-[1600px] mx-auto space-y-6 pb-20 pt-2">
                ...
                {isLoading ? (
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 p-16 text-center shadow-sm">
                        ...
                        <Button variant="outline" className="mt-6 rounded-xl font-black uppercase text-[10px] text-[#4B7BA7] border-blue-200 hover:bg-blue-50" onClick={() => navigate('/parroquia/confirmacion/sentar-registros')}>
                    ...
                    <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-blue-900/5 border border-slate-100 overflow-hidden">
                        <Table 
                            columns={columns} 
                            data={records} 
                            onRowClick={(row) => setSelectedPartida(current => current?.id === row.id ? null : row)}
                            actions={[
