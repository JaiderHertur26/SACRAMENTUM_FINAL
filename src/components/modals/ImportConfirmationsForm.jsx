...
const ImportConfirmationsForm = ({ isOpen, onClose }) => {
  const { user } = useAuth();
  const { importConfirmations, validateJSONStructure } = useAppData();
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = async (event) => {
        try {
            ...
            setJsonContent(json);
            const result = await importConfirmations(json, user?.parishId, true);
  ...
  const handleConfirm = async () => {
      ...
      setLoading(true);
      const result = await importConfirmations(jsonContent, user?.parishId, false);
      setLoading(false);
  ...
  return (
    <Modal size="lg" isOpen={isOpen} onClose={handleClose} title="Importar Confirmaciones">
        <div className="space-y-6 w-full">
            {!preview && (
                <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center bg-slate-50 hover:bg-slate-100 transition-colors">
                    <input type="file" accept=".json" onChange={handleFileChange} className="hidden" id="conf-upload" />
                    <label htmlFor="conf-upload" className="cursor-pointer flex flex-col items-center gap-2">
            ...
            <div className="flex justify-end gap-3 pt-2 border-t">
                ...
                <Button onClick={handleConfirm} disabled={!validationResult || validationResult.count === 0} className="bg-red-600 hover:bg-red-700 text-white">
...
export default ImportConfirmationsForm;