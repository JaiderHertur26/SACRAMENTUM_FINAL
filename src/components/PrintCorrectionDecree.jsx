...
const PrintCorrectionDecree = forwardRef(({ decreeData }, ref) => {
  ...
  const cancillerName = decreeData.cancillerName || decreeData.chancellorName || chanceryData.canciller || chanceryData.nombreCanciller || chanceryData.parroco || '[CANCILLER NO CONFIGURADO]';
  const cancillerCargo = decreeData.cancillerCargo || decreeData.chancellorTitle || chanceryData.cargo || 'CANCILLER DIOCESANO';
  ...
  baptismRecord.tipoUnionPadres = unionTypeMap[String(baptismRecord.tipoUnionPadresRaw)] || baptismRecord.tipoUnionPadresRaw || '';
  const sexoNormalizado = String(baptismRecord.sexoRaw || '').trim().toUpperCase();
  baptismRecord.sexo = ['1', 'M', 'MASCULINO'].includes(sexoNormalizado)
    ? 'Masculino'
    : ['2', 'F', 'FEMENINO'].includes(sexoNormalizado)
      ? 'Femenino'
      : '';
      ...
      <div className="w-full flex-1 flex flex-col">
          ...
          <div className="mb-2 text-[9pt] text-black shrink-0">
            <p className="text-justify leading-snug">
              Por el presente documento, el Gobierno de la {jurisdictionKind} ordena y autoriza la anulación de la Partida de BAUTISMO correspondiente a:
            </p>
            ...
            <div className="flex flex-col flex-1 justify-between pt-1">
              ...
              <div className="flex w-full">
                <div className="w-[55%] flex items-end pr-1">
                    ...
                    <span className="font-mono flex-1 border-b border-dotted border-slate-500 pl-1 uppercase text-[8.5pt] text-black">{baptismRecord.tipoUnionPadres}</span>
                </div>
                <div className="w-[45%] flex items-end pl-1">
                    ...
                    <span className="font-mono flex-1 border-b border-dotted border-slate-500 pl-1 uppercase text-[8.5pt] text-black">{baptismRecord.sexo}</span>
                </div>
              </div>
