...
declare
  ...
  v_catechumen_stats jsonb;
  v_marriage_status jsonb;
  v_marriage_categories jsonb;
begin
  ...
  with base as (
    select
      public.sacramentum_age_years(b.fecha_nacimiento, b.celebration_date) age_years,
      lower(trim(coalesce(
        b.raw_data->>'catechumenPreparationStatus',
        b.raw_data->>'catechumen_preparation_status',
        ''
      ))) preparation_status
    from public.baptisms b
    where b.parish_id = any(v_parish_ids)
  ...
  select jsonb_build_object(
    'prepared_over7', count(*) filter (
      where age_years > 7 and preparation_status = 'prepared'
    ),
    'not_prepared_over7', count(*) filter (
      where age_years > 7 and preparation_status = 'not_prepared'
    ),
    'unknown_over7', count(*) filter (
      where age_years > 7
        and preparation_status not in ('prepared','not_prepared')
    )
  )
  into v_catechumen_stats
  from base;
  ...
  with base as (
    select lower(trim(coalesce(
      m.raw_data->>'canonicalMarriageCategory',
      m.raw_data->>'canonical_marriage_category',
      ''
    ))) category
    from public.marriages m
    where m.parish_id = any(v_parish_ids)
  ...
  select jsonb_build_object(
    'both_catholic_baptized', count(*) filter (where category = 'both_catholic_baptized'),
    'mixed_marriage', count(*) filter (where category = 'mixed_marriage'),
    'disparity_of_cult', count(*) filter (where category = 'disparity_of_cult'),
    'unclassified', count(*) filter (
      where category not in ('both_catholic_baptized','mixed_marriage','disparity_of_cult')
    )
  )
  into v_marriage_categories
  from base;
  ...
  return jsonb_build_object(
    ...
    'baptism_catechumen_stats', coalesce(v_catechumen_stats, '{}'::jsonb),
    'marriage_baptism_status', coalesce(v_marriage_status, '{}'::jsonb),
    'marriage_canonical_categories', coalesce(v_marriage_categories, '{}'::jsonb)
  );
end;
$$;
...
is 'Desglose uniforme para reporte a la Curia, incluida situación catecumenal y clasificación canónica matrimonial registradas en los sacramentos.';
